...
const copy = {
  en: { eyebrow: 'VALORANT GAMEPLAY ROASTER', brandLead: 'Clutch', brandTail: 'or Cringe', tagline: 'Your stats entered the site. Your dignity did not.', intro: 'Real match stats. Deterministic analysis. One AI-powered roast—no invented facts.', riotId: 'Riot ID', placeholder: 'Name#TAG', intensity: 'Heat level', submit: 'Roast me', mild: 'Mild', spicy: 'Spicy', brutal: 'Brutal', loadingPlayer: 'Scouting your match history…', loadingRoast: 'Preheating the roast…', retry: 'Try another player', stats: 'Receipts', matches: 'Matches', wins: 'Wins', rank: 'Rank', agent: 'Most played agent', agentArtwork: 'Agent artwork', rankArtwork: 'Rank emblem', kd: 'K/D', hs: 'HS%', wr: 'Win rate', acs: 'ACS', adr: 'ADR', invalid: 'Enter a valid Riot ID in Name#TAG format.', generic: 'The server missed its shot. Try again.', notFound: 'No recent competitive matches were found.', rateLimited: 'The stats provider needs a timeout. Try again shortly.' },
  es: { eyebrow: 'ROASTER DE GAMEPLAY DE VALORANT', brandLead: 'Clutch', brandTail: 'or Cringe', tagline: 'Tus stats entraron al site. Tu dignidad no.', intro: 'Estadísticas reales. Análisis determinista. Un roast con IA, sin datos inventados.', riotId: 'Riot ID', placeholder: 'Nombre#TAG', intensity: 'Nivel de fuego', submit: 'Hazme roast', mild: 'Suave', spicy: 'Picante', brutal: 'Brutal', loadingPlayer: 'Revisando tu historial…', loadingRoast: 'Precalentando el roast…', retry: 'Probar otro jugador', stats: 'Las pruebas', matches: 'Partidas', wins: 'Victorias', rank: 'Rango', agent: 'Agente más jugado', agentArtwork: 'Arte del agente', rankArtwork: 'Emblema de rango', kd: 'K/D', hs: 'HS%', wr: 'Winrate', acs: 'ACS', adr: 'ADR', invalid: 'Escribe un Riot ID válido con formato Nombre#TAG.', generic: 'El servidor falló el disparo. Intenta otra vez.', notFound: 'No encontramos partidas competitivas recientes.', rateLimited: 'El proveedor necesita un descanso. Intenta en un momento.' },
} as const
...
function RoastCard({ stats, roast, locale }: { stats: PlayerStats; roast: RoastResult; locale: Locale }) {
  const t = copy[locale]
  const featuredAgent = stats.mainAgent ?? stats.agents[0]
  const visual = stats.avatarUrl ?? featuredAgent?.imageUrl ?? stats.rankImageUrl
  const visualLabel = stats.avatarUrl ? undefined : featuredAgent?.imageUrl ? t.agentArtwork : stats.rankImageUrl ? t.rankArtwork : undefined
  return <article className="roast-card" aria-labelledby="roast-title">
    <div className="card-glow" />
    <header className="player-header">
      <div className="player-visual">
        {visual ? <img src={visual} alt={visualLabel ?? stats.riotId} className="player-art" /> : <div className="player-monogram" aria-hidden="true">{stats.name[0]}</div>}
...
export default function App() {
  ...
  return <main>
    <div className="arena-background" aria-hidden="true"><div className="arena-grid" /><div className="scan-beam" />{Array.from({ length: 12 }, (_, index) => <i className={`ember ember-${index + 1}`} key={index} />)}</div>
    ...
    <section className="hero"><p className="eyebrow">{t.eyebrow}</p><h1><span>{t.brandLead}</span> {t.brandTail}</h1><p className="tagline">{t.tagline}</p><p className="intro">{t.intro}</p>
